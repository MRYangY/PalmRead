package com.example.yangyu.palmread.Constant;

/**
 * Created by yangyu on 2017/1/16.
 */

public class ProjectContent {
    public static String mAppid = "1106227339";
    public static String EXTRA_HOME_FRAGMENT_RESULT="HomeFragmentResultTAG";
    public static String EXTRA_TAB_INDEX="TabIndexTAG";
    public static String EXTRA_VIDEO_INDEX="ExtraVideoIndexTAG";
    public static String EXTRA_VIDEO_URL="ExtraVideoUrlTAG";
    public static String EXTRA_VIDEO_PLAY_URL="ExtraVideoUrlTAG";
    public static String EXTRA_VIDEO_USER_NAME="ExtraVideoUserNameTAG";
    public static String EXTRA_QQ_USER_INFO="qq_user_info";
    public static final String EXTRA_TO_PERSON="ExtraToPersonTAG";
    public static final String ACTION_NEWS_COLLECT_CHANGE="news_collect_change";
    public static final String ACTION_NEWS_HISTORY_CHANGE="news_history_change";
//    public static final String ACTION_VIDEO_COLLECT_CHANGE="video_collect_change";
//    public static final String ACTION_VIDEO_HISTORY_CHANGE="video_history_change";
    public static final String USER_ACCOUNT="user_account";
    public static final String USER_PASSWORD="user_password";
    public static final String REGISTER_OK="register_ok";
    public static final String LOGIN_OK="login_ok";
    public static final String INTENT_LOGIN_OK="action_login_ok";
    public static final String INTENT_LOGOUT_OK="action_logout_ok";
    public static final String USER_INFO_NAME="user_info_name";
    public static final String USER_INFO_PIC="user_info_pic";
    public static final String USER_INFO_INTRODUCE="user_info_introduce";
}
